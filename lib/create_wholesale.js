...
var populateAll = require('./populateAll');
exports = module.exports = function create_wholesale(wholesale_form, req, next, locals){
                    ...
                    if (wholesale_form.num > product.num){
                        ...
                        keystone.list('Product').model.findByIdAndUpdate(product._id, {$set:{num: 0}}, function(err, result){
                            ...
                            item.save(function(err, item_result){
                                //Push to member's channel
                                if(user_room[wholesale_form.user_id]){
                                    user_room[wholesale_form.user_id].emit('new_transaction', {
                                ...
                                q_for_book.exec(function(err, book_result){
                                    if(book_result){
                                        keystone.list('Book').model.findByIdAndUpdate(book_result._id, {$set:{num: parseFloat(book_result.num) + parseFloat(product.num)}}, function (err, item_result){
                                            if (user_room[wholesale_form.user_id]){
                                                var q_for_member = keystone.list('User').model.findOne({'_id': wholesale_form.user_id});
                                                q_for_member.exec(function(err, member){
                                                    var q_for_books = keystone.list('Book').model.find({'location': member.my_warehouse});
                                                    q_for_books.exec(function(err, books){
                                                        if (books){
                                                            populateAll(books, 'product', 'Product', function(){
                                                                if(user_room[wholesale_form.user_id])
                                                                user_room[wholesale_form.user_id].emit('new_book_list', books);

                                                                callback(null, 'transaction_generated');
                                                            })
                                                        }else{
                                                            if(user_room[wholesale_form.user_id])
                                                            user_room[wholesale_form.user_id].emit('new_book_list', null);
                        ...
                        keystone.list('Product').model.findByIdAndUpdate(product._id, {$set:{num: product.num - wholesale_form.num}}, function(err, result){
                            ...
                            item.save(function(err, item_result){
                                ...
                                q_for_book.exec(function(err, book_result){
                                    if(book_result){
                                        keystone.list('Book').model.findByIdAndUpdate(book_result._id, {$set:{num: parseFloat(book_result.num) + parseFloat(wholesale_form.num)}}, function (err, item_result){
                                            if (user_room[wholesale_form.user_id]){
                                                var q_for_member = keystone.list('User').model.findOne({'_id': wholesale_form.user_id});
                                                q_for_member.exec(function(err, member){
                                                    var q_for_books = keystone.list('Book').model.find({'location': member.my_warehouse});
    ...
    async.waterfall([
        ...
        if (feedback == 'transaction_generated'){
            req.flash('info', '购买成功');
            console.log('End of create_wholesale');
            return next();
        }else{
            req.flash('error', feedback);
            return next();
        }
    }); 
}