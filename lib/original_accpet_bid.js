...
exports = module.exports = function accept_bid(product){
    ...
    function check_out(feedback, condition, callback){
        if (condition){
            var query_for_bidder = keystone.list('User').model.findOne({'_id': product.biddings[0].bidder});
            query_for_bidder.exec(function(err, bidder){
                if (product.biddings[0].num * product.biddings[0].price <= bidder.capital){
                    keystone.list('User').model.findByIdAndUpdate(bidder._id, {$set:{capital: bidder.capital - product.biddings[0].num * product.biddings[0].price}}, function (err, result){
    ...
    function transaction_generation(feedback, available, affordable, callback){
        if (available){
            if (affordable){
                ...
                callback(null, feedback, false, true);
            }
        }else{
